...
pub fn fetch(id: u32, server: &str) -> Option<Vec<u8>> {
  let addr = parse_socket(server);
  let sock = UdpSocket::bind((Ipv4Addr::UNSPECIFIED, 0)).ok()?;
  sock.connect(addr).ok()?;
  sock.set_read_timeout(Some(Duration::from_secs(2))).ok()?;
  let init = fetch_slice(&sock, message::request(id, 1))?;
  if init.last_slice == 0 {
    return Some(vec![]);
  }
  if init.last_slice == 1 {
    return Some(init.body);
  }
  ...
  for i in 2..(slice_cnt - 1) {
    let response =
      fetch_slice(&sock, message::request(id, i.try_into().ok()?))?;
    buf[((i - 1) * SLICE_SIZE)..(i * SLICE_SIZE)]
      .clone_from_slice(&response.body)
  }
